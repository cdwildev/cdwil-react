  ...
  return (
    <ContainerUI>
      ...
      <TextContainerUI bachelor={bachelor}>
          ...
          <TextUI> <PointContainerUI> <PointUI style={{background: bachelor.length > 0 ? 'white' : 'transparent'}}> </PointUI><LineUI style={{height: skill.length > 0 ? '150px' : '0px' }}></LineUI></PointContainerUI> {bachelor.length > 0 ? bachelor.replace('-', ' ').replace(/(?:^|\s)\S/g, a => a.toUpperCase()) : <div>Select Your <span style={{color: '#00B188'}}>Degree</span></div>}</TextUI>
          <TextUI style={{opacity: bachelor.length > 0 ? '100%' : '0' }}> <PointContainerUI><PointUI style={{background: major.length > 0 ? 'white' : 'transparent'}}/> </PointContainerUI>{major == 'film-and-screen-art' ? 'Film + Screen Art' : major == 'fmsa' ? 'New Media + Sound Art' : major == 'crcp' ? 'Critical + Cultural Practise' : major.length > 0 ? major.replace('-', ' ').replace(/(?:^|\s)\S/g, a => a.toUpperCase()) : <div>Select Your <span style={{color: '#EDE04A'}}>Major</span></div>}</TextUI>
